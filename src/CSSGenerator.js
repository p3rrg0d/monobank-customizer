/**
 * CSSGenerator - Generates custom CSS from widget state
 * Handles CSS generation and export functionality
 */

import { hexToRgba } from './utils.js';

/**
 * Get background CSS based on type (solid or gradient)
 * @param {string} type - 'solid' or 'gradient'
 * @param {string} solidColor - Hex color for solid type
 * @param {number} solidOpacity - Opacity for solid type (0-1)
 * @param {string} gradString - Gradient CSS string for gradient type
...
/**
 * QR Frame SVG definitions
 * ⚠️ УВАГА: Додай сюди свої SVG рамки як base64 data URIs
 * Формат: 'frame_id': 'url(data:image/svg+xml;base64,...)'
 */
const QR_FRAMES = {
    ...
    // Рамка без кота (frame2)
    // 👇 ЗАМІНИ ЦЕЙ BASE64 НА СВІЙ SVG
...
/**
 * Get QR frame SVG as CSS url()
 * @param {string} frameId - ID of the frame ('standard', 'frame1', 'frame2', etc.)
 * @returns {string} CSS url() value with SVG
 */
...
/**
 * Generate custom CSS from widget state
...
export function generateWidgetCSS(state) {
    ...
    // Get QR frame SVG (without url() wrapper for CSS output)
    const qrFrameUrl = getQRFrameSVG(state.qrFrame);
    // Extract just the data URI from url(...)
    const qrFrameDataUri = qrFrameUrl.replace(/^url\(/, '').replace(/\)$/, '');

    // Спеціальні стилі QR тільки для frame2
...
/**
 * CSSExporter - Manages CSS export UI and functionality
 */
export class CSSExporter {
    ...
    /**
     * Update the CSS export display
     * @param {string} css - CSS string to display
     */
    updateCSS(css) {
        if (!this.initialized) {
            this.initialize();
