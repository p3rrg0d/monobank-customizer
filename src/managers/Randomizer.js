...
export class Randomizer {
    ...
    static jitterColor(hex, amount = 10) {
        try {
    ...
    static generate() {
        ...
        // === РЕДАГУВАННЯ ШАНСІВ (у відсотках %) ===
        const CHANCE_GLASS = 80;    // Шанс на прозорість
        const CHANCE_BORDER = 10;   // Шанс на бордер
        const CHANCE_SHADOW = 25;   // Шанс на тінь тексту
        const CHANCE_GRADIENT = 65; // Шанс на градієнт фону
        const CHANCE_PROG_GRAD = 35;// Шанс на градієнт прогресу
        const CHANCE_STEPPED = 15;  // Шанс на "смугастий" градієнт
        const CHANCE_JITTER = 85;   // Шанс на легку зміну відтінків палітри
        const CHANCE_TRANS_GRAD = 90; // Шанс на прозорість окремих точок градієнта
        // =========================================

        const isGlassy = chance(CHANCE_GLASS);
        const isStepped = chance(CHANCE_STEPPED);
        const hasTransGrad = chance(CHANCE_TRANS_GRAD);

        const getBaseColors = () => {
            let colors = [...palette.bg];
            // Додаємо акцентний колір у 40% випадків для цікавості
        ...
        // 1. Background Logic
        // Базова прозорість для солідів або звичайних градієнтів
        const baseBgOpacity = isGlassy ? randomFloat(0.2, 0.5) : randomFloat(0.8, 1);
        ...
        if (chance(CHANCE_GRADIENT)) {
            ...
            const stops = [];
            if (isStepped) {
                // Смугастий режим (Sharp transitions)
                ...
                // Плавний режим з нерівномірними кроками
                colors.forEach((color, i) => {
                    let pos;
                    if (i === 0) pos = 0;
                    ...
                    // Якщо увімкнено transGrad, кожна точка отримує свою прозорість (ефект затухання)
                    const op = hasTransGrad ? randomFloat(0.1, 1) : baseBgOpacity;
                    stops.push({ id: i + 1, color, opacity: op, position: pos });
                });
            }

            overrides.bgGradientStops = stops.sort((a, b) => a.position - b.position);
        ...
        // 2. Text & Effects
        overrides.textColor = chance(CHANCE_JITTER) ? this.jitterColor(palette.text, 5) : palette.text;
        ...
        if (overrides.textShadowEnabled) {
            ...
            overrides.textShadowColor = chance(CHANCE_JITTER) ? this.jitterColor(shadowBaseColor) : shadowBaseColor;
            overrides.textShadowOpacity = randomFloat(0.5, 1);
            overrides.textShadowBlur = shadowType === 'brutal' ? 0 : randomInt(5, 20);
            overrides.textShadowX = shadowType === 'brutal' ? randomChoice([-3, 3]) : 0;
            overrides.textShadowY = shadowType === 'brutal' ? randomChoice([-3, 3]) : (shadowType === 'soft' ? 4 : 0);
        }

        // 3. Progress Bar
        overrides.progressRadius = randomChoice([0, 4, 8, 12, 25, 50]);
        overrides.progTrackType = "solid";
        overrides.progTrackSolidColor = palette.text === '#ffffff' ? '#ffffff' : '#000000';
        ...
        if (overrides.progFillType === "solid") {
            overrides.progFillSolidColor = chance(CHANCE_JITTER) ? this.jitterColor(palette.accent) : palette.accent;
            ...
            overrides.progFillGradientString = generateGradientString(overrides.progFillGradientStops, overrides.progFillGradientAngle);
        }

        // 4. Structural Randomness (Radii & Borders)
        overrides.borderRadius = randomChoice([0, 0, 4, 8, 12, 16, 20, 24, 32, 48, 64, 100]);
        overrides.borderEnabled = chance(CHANCE_BORDER);
        if (overrides.borderEnabled) {
            overrides.borderStyle = randomChoice(["solid", "solid", "dashed", "dotted", "double"]);
            overrides.borderWidth = randomInt(1, 8);
            overrides.borderColor = Math.random() > 0.5 ? palette.text : palette.accent;
            if (chance(CHANCE_JITTER)) overrides.borderColor = this.jitterColor(overrides.borderColor);
            overrides.borderOpacity = isGlassy ? randomFloat(0.3, 0.7) : randomFloat(0.2, 1);
        } else {
