import { hexToRgba } from './helpers.ts';
import confetti from 'canvas-confetti';
import { FillSettings } from '../data/types.ts';

const MONO_COLORS = ['#ff5f57', '#febc2e', '#483bce', '#28c840'];

export function triggerCelebration() {
    const duration = 2 * 1000;
    const animationEnd = Date.now() + duration;

    const defaults: any = {
        startVelocity: 45,
        spread: 360,
        ticks: 60,
        zIndex: 9999,
        colors: MONO_COLORS,
        shapes: ['square'],
        scalar: 0.6
    };

    function randomInRange(min: number, max: number) {
        return Math.random() * (max - min) + min;
    }

    confetti({
        ...defaults,
        particleCount: 50,
        angle: 60,
        spread: 55,
        origin: { x: -0.1, y: 0.5 },
        drift: 0.5
    });
    confetti({
        ...defaults,
        particleCount: 50,
        angle: 120,
        spread: 55,
        origin: { x: 1.1, y: 0.5 },
        drift: -0.5
    });

    setTimeout(() => {
        confetti({
            ...defaults,
            particleCount: 120,
            spread: 80,
            origin: { y: 0.7 },
            scalar: 0.8
        });
    }, 200);

    const interval = setInterval(function () {
        const timeLeft = animationEnd - Date.now();

        if (timeLeft <= 0) {
            return clearInterval(interval);
        }

        const particleCount = 25 * (timeLeft / duration);
        confetti({
            ...defaults,
            particleCount,
            origin: {
                x: randomInRange(0.1, 0.4),
                y: Math.random() - 0.2
            }
        } as any);
        confetti({
            ...defaults,
            particleCount,
            origin: {
                x: randomInRange(0.6, 0.9),
                y: Math.random() - 0.2
            }
        } as any);
    }, 250);
}

export function getBackgroundCSS(fill: FillSettings): string {
    if (fill.type === "solid") {
        return hexToRgba(fill.color, fill.opacity);
    }
    return fill.gradientString || '';
}

const QR_FRAMES: Record<string, string> = {
    standard: `url(data:image/svg+xml;base64,)`,

    frame1: `url(data:image/svg+xml;base64,)`,

    frame2: `url(data:image/svg+xml;base64,)`,

    frame3: `url(data:image/svg+xml;base64,)`
};
export function getQRFrameSVG(frameId: string): string {
    return QR_FRAMES[frameId] || QR_FRAMES.standard;
}

export function generateWidgetCSS(state: any): string {
    const background = getBackgroundCSS(state.background);
    const border = state.border.enabled
        ? `${state.border.width}px ${state.border.style} ${hexToRgba(state.border.color, state.border.opacity)}`
        : 'none';

    const trackBg = getBackgroundCSS(state.progress.track);
    const fillBg = getBackgroundCSS(state.progress.fill);

    const qrFrameUrl = getQRFrameSVG(state.qrFrame);
    const qrFrameDataUri = qrFrameUrl.replace(/^url\(/, '').replace(/\)$/, '');

    let qrFrameStyling = '';
    if (state.qrFrame === 'frame2') {
        qrFrameStyling = `
.widget.qr-container .qr {
    width: 50% !important;
}
.widget.qr-with-progress-widget .qr-side .widget.qr-container .qr {
    top: 17px !important;
    left: 13.4% !important;
    position: relative !important;
}
.widget.qr-with-progress-widget .qr-side .widget.qr-container {
    width: 180px !important;
    background-size: 100% auto !important;
    background-position-x: -20.1px !important;
    background-position-y: -19.5px !important;
    background-repeat: no-repeat !important;
    background-image: url(${qrFrameDataUri}) !important;
}`;
    } else {
        qrFrameStyling = `
.widget.qr-with-progress-widget .qr-side .widget.qr-container {
    background-image: url(${qrFrameDataUri}) !important;
}`;
    }

    const textShadow = state.text.shadow.enabled
        ? `${state.text.shadow.x}px ${state.text.shadow.y}px ${state.text.shadow.blur}px ${hexToRgba(state.text.shadow.color, state.text.shadow.opacity)}`
        : 'none';

    return `
.widget.qr-with-progress-widget.color-scheme-black {
    border-radius: ${state.border.radius}px !important;
    background: ${background} !important;
    border: ${border} !important;
    color: ${state.text.color} !important;
}

.widget.qr-with-progress-widget .title,
.widget.qr-with-progress-widget .progress-container .widget .text,
.linear-horizontal-progress-widget .text .text-balance,
.linear-horizontal-progress-widget .text .text-goal {
    color: ${state.text.color} !important;
    text-shadow: ${textShadow} !important;
}

.linear-horizontal-progress-widget.shape-scheme-rounded .background,
.linear-horizontal-progress-widget.shape-scheme-rounded .progress-clip,
.linear-horizontal-progress-widget.shape-scheme-rounded .progress {
    border-radius: ${state.progress.radius}px !important;
}

.linear-horizontal-progress-widget .background {
    background: ${trackBg} !important;
}

.linear-horizontal-progress-widget.color-scheme-pink .progress {
    background: ${fillBg} !important;
}
${qrFrameStyling}

.widget.qr-container svg path {
    fill: #ffffff !important;
}
    `.trim();
}

export interface ICSSExporter {
    updateCSS(css: string): void;
}

export class CSSExporter implements ICSSExporter {
    private cssExportElement: HTMLElement;
    private onFirstCopy?: () => void;
    private onTutorialClick?: () => void;
    private codeBlock: HTMLElement | null = null;
    private initialized: boolean = false;

    constructor(cssExportElement: HTMLElement, onFirstCopy?: () => void, onTutorialClick?: () => void) {
        this.cssExportElement = cssExportElement;
        this.onFirstCopy = onFirstCopy;
        this.onTutorialClick = onTutorialClick;
    }

    initialize() {
        if (this.initialized) return;

        this.cssExportElement.innerHTML = `
            <div class="code-title">
                <span>CSS код</span>
            </div>
            <pre></pre>
            <div style="display: flex; gap: 10px; margin-top: 24px;">
                <button id="share-btn" class="tutorial-btn-secondary" title="Поділитись дизайном" style="margin-top: 0; background: #febc2e; border-color: #000;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        <circle cx="18" cy="5" r="3"/>
                        <circle cx="6" cy="12" r="3"/>
                        <circle cx="18" cy="19" r="3"/>
                        <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>
                        <line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
                    </svg>
                </button>
                <button class="copy-btn-wide" id="copyBtn" style="margin-top: 0; flex-grow: 1;">Скопіювати CSS код</button>
                <button id="tutorial-btn" class="tutorial-btn-secondary" title="Інструкція" style="margin-top: 0;">?</button>
            </div>
        `;

        this.codeBlock = this.cssExportElement.querySelector("pre");
        this.initCopyButton();
        this.initTutorialButton();
        this.initialized = true;
    }

    private initTutorialButton() {
        const tutorialBtn = this.cssExportElement.querySelector("#tutorial-btn");
        if (!tutorialBtn) return;

        tutorialBtn.addEventListener("click", () => {
            if (this.onTutorialClick) {
                this.onTutorialClick();
            }
        });
    }

    private initCopyButton() {
        const copyBtn = this.cssExportElement.querySelector("#copyBtn");
        const mobileCopyBtn = document.getElementById("mobile-copy-css");

        const copyHandler = () => {
            if (!this.codeBlock) return;
            navigator.clipboard.writeText(this.codeBlock.textContent || '').then(() => {
                if (!localStorage.getItem('monobank_widget_css_copied')) {
                    localStorage.setItem('monobank_widget_css_copied', 'true');
                    if (this.onFirstCopy) {
                        this.onFirstCopy();
                    }
                }

                if (copyBtn instanceof HTMLButtonElement) {
                    const originalText = copyBtn.textContent;
                    copyBtn.textContent = "Скопійовано!";
                    copyBtn.style.background = "var(--accent-secondary)";
                    copyBtn.style.color = "var(--text-main)";

                    setTimeout(() => {
                        copyBtn.textContent = originalText;
                        copyBtn.style.background = "";
                        copyBtn.style.color = "";
                    }, 2000);

                    triggerCelebration();
                }

                if (mobileCopyBtn instanceof HTMLButtonElement) {
                    const originalText = mobileCopyBtn.textContent;
                    mobileCopyBtn.textContent = "✓";
                    mobileCopyBtn.style.background = "var(--accent-secondary)";
                    mobileCopyBtn.style.color = "var(--text-main)";

                    setTimeout(() => {
                        mobileCopyBtn.textContent = originalText;
                        mobileCopyBtn.style.background = "";
                        mobileCopyBtn.style.color = "";
                    }, 2000);
                }

                const tutorialBtn = document.getElementById("tutorial-btn");
                if (tutorialBtn) {
                    tutorialBtn.classList.add("tutorial-highlight");
                    setTimeout(() => {
                        tutorialBtn.classList.remove("tutorial-highlight");
                    }, 3000);
                }
            });
        };

        if (copyBtn) {
            copyBtn.addEventListener("click", copyHandler);
        }

        if (mobileCopyBtn) {
            mobileCopyBtn.addEventListener("click", copyHandler);
        }
    }

    updateCSS(css: string) {
        if (!this.initialized) {
            this.initialize();
        }
        if (this.codeBlock) {
            this.codeBlock.textContent = css;
        }
    }
}
